    ...
    <div class="applicantGenerals view large-9 medium-8 columns content">
        <div class ='col-md-6'>
            <p>
                <?php
                echo $this->Html->link(
                        $this->Html->tag('span', '', ['class' => 'glyphicon glyphicon-edit']) . ' ' . __('Edit'), ['controller' => 'foundtems', 'action' => 'edit', $foundtems->id], ['class' => 'btn btn-info', 'role' => 'button', 'escape' => false]
                );
                ?>
            </p> 
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    <table class="table vertical-table">
                        ...
                        <tr>
                            <th><?= __('Name') ?></th>
                            <td><?= h($foundtems->name) ?></td>
                        </tr>

                        <tr>
                            <th><?= __('Losting Date') ?></th>
                             <td><?= h($foundtems->lost_date) ?></td>                      
                        </tr>                            

                        <tr>
                            <th><?= __('country') ?></th>
                            <td><?= $foundtems->has('country') ? h($foundtems->country->name_en) : 'no' ?></td>
                        </tr>
                        <tr>
                            <th><?= __('Item Type') ?></th>
                            <td><?= $foundtems->has('item_type_id') ? h($foundtems->item_type->name) : '' ?></td>
                        </tr>
                        <tr>
                            <th><?= __('Created') ?></th>
                            <td><?= h($foundtems->created) ?></td>
                        </tr>
                        <tr>
                            <th><?= __('Modified') ?></th>
                            <td><?= h($foundtems->modified) ?></td>
